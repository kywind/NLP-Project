...
import log
from pet.config import AdversarialConfig, EvalConfig, TrainConfig
from pet.utils import InputExample, exact_match, save_logits, save_predictions, softmax, LogitsList, set_seed, eq_div
from pet.wrapper import TransformerModelWrapper, RobustWrapper, Adversarial
from pet.config import  WrapperConfig, AdversarialConfig
from pet.adversarial import RandomPerturbationAdversarial, HotFlipAdversarial, InputSpecificAdversarial, FlipLabelAdversarial
...
def init_adv(config: AdversarialConfig):
    return getattr(sys.modules[__name__], config.method_name)(config)


def init_model_adv(config: WrapperConfig, adv_config: AdversarialConfig) -> RobustWrapper:
    """Initialize a new model from the given config."""
    assert config.pattern_id is not None, 'A pattern_id must be set for initializing a new PET model'
    model = RobustWrapper(config, init_adv((adv_config)))
    model.adversarial.wrapper = model
    return model
...
def train_pet_adv(train_data: List[InputExample],
              eval_data: List[InputExample],
              dev32_data: List[InputExample],
              model_config: WrapperConfig,
              train_config: TrainConfig,
              ...
              repetitions: int = 3,
              do_train: bool = True,
              do_train_adv: bool = True,
              do_eval: bool = True,
              do_eval_adv: bool = True,
              seed: int = 42
              ):
    ...
    results = defaultdict(lambda: defaultdict(list))
    dev32_results = defaultdict(lambda: defaultdict(list))
    results_adv = defaultdict(lambda: defaultdict(list))
    dev32_results_adv = defaultdict(lambda: defaultdict(list))
    set_seed(seed)
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            wrapper = init_model_adv(model_config, adv_config)

            # Training
            if do_train:
                if do_train_adv:
                    train_dict = train_single_model_adv(train_data, eval_data, dev32_data, pattern_iter_output_dir, \
                                                       wrapper, train_config, eval_config, adv_config)
                else:
                    train_dict = train_single_model(train_data, eval_data, dev32_data, pattern_iter_output_dir, \
                                                       wrapper, train_config, eval_config)
                results_dict.update(train_dict)
                results_dict_adv.update(train_dict)
                ...
                train_config.save(os.path.join(pattern_iter_output_dir, 'train_config.json'))
                eval_config.save(os.path.join(pattern_iter_output_dir, 'eval_config.json'))
                adv_config.save(os.path.join(pattern_iter_output_dir, 'adv_config.json'))
                logger.info("Saving complete")
            ...
            if do_eval:
                ...
                # if not wrapper:
                wrapper = RobustWrapper.from_pretrained(pattern_iter_output_dir)
                if eval_config.eval_data_selection == 'all':
                    eval_result = evaluate(wrapper, eval_data, eval_config)
                    dev32_result = evaluate(wrapper, dev32_data, eval_config)
                else:
                    eval_data_select = []
                    for example in eval_data:
                        if example.label == eval_config.eval_data_selection:
                            eval_data_select.append(example)
                    dev32_data_select = []
                    for example in dev32_data:
                        if example.label == eval_config.eval_data_selection:
                            dev32_data_select.append(example)
                    eval_result = evaluate(wrapper, eval_data_select, eval_config)
                    dev32_result = evaluate(wrapper, dev32_data_select, eval_config)
            ...
            if do_eval_adv:
                ...
                # if not wrapper:
                wrapper = RobustWrapper.from_pretrained(pattern_iter_output_dir)
                wrapper.adversarial = init_adv((adv_config))
                wrapper.adversarial.wrapper = wrapper


                if not wrapper.adversarial.no_training:
                    if eval_config.eval_data_selection == 'all':
                        wrapper.train_adv(eval_data, train_config.adv_train_batch_size, train_config.adv_train_epochs, adv_config.adv_lr, adv_config.adv_weight_decay)
                    else:
                        train_data_select = []
                        for example in train_data:
                            if example.label == eval_config.eval_data_selection:
                                train_data_select.append(example)
                        wrapper.train_adv(train_data_select, train_config.adv_train_batch_size, train_config.adv_train_epochs, adv_config.adv_lr, adv_config.adv_weight_decay)

                # trivial
                # wrapper.adversarial.attack1 = 'the the the'
                # wrapper.adversarial.attack2 = 'the the the'

                # xlarge, seed=42, nofewshot
                # wrapper.adversarial.attack1 = 'none referencecoming'
                # wrapper.adversarial.attack2 = 'things middleweight cole'

                # xxlarge, seed=1, fewshot
                # wrapper.adversarial.attack1 = 'nearlypl protection'
                # wrapper.adversarial.attack2 = 'emile 201718 shouted'
                
                # print(wrapper.adversarial.config.method_name)

                if wrapper.adversarial.config.method_name == 'HotFlipAdversarial':
                    print('\n')
                    print(wrapper.adversarial.attack1)
                    print(wrapper.adversarial.attack2)
                    print('\n')

                if eval_config.eval_data_selection == 'all':
                    eval_result = evaluate_adv(wrapper, eval_data, eval_config)
                    dev32_result = evaluate_adv(wrapper, dev32_data, eval_config)
                else:
                    eval_data_select = []
                    for example in eval_data:
                        if example.label == eval_config.eval_data_selection:
                    ...
                    for example in dev32_data:
                        if example.label == eval_config.eval_data_selection:
                            dev32_data_select.append(example)
                    eval_result = evaluate_adv(wrapper, eval_data_select, eval_config)
                    dev32_result = evaluate_adv(wrapper, dev32_data_select, eval_config)

                save_predictions(os.path.join(pattern_iter_output_dir, 'eval_predictions_adv.jsonl'), wrapper, eval_result)
                save_logits(os.path.join(pattern_iter_output_dir, 'eval_logits_adv.txt'), eval_result['logits'])

                save_predictions(os.path.join(pattern_iter_output_dir, 'dev32_predictions_adv.jsonl'), wrapper, dev32_result)
                save_logits(os.path.join(pattern_iter_output_dir, 'dev32_logits_adv.txt'), dev32_result['logits'])

                logger.info("--- RESULT ADV (pattern_id={}, iteration={}) ---".format(pattern_id, iteration))
                logger.info("eval_adv_results:")
                logger.info(eval_result['scores'])
                logger.info("dev32_adv_results:")
                logger.info(dev32_result['scores'])

                results_dict['eval_set_after_training'] = eval_result['scores']
                results_dict['dev32_set_after_training'] = dev32_result['scores']
                with open(os.path.join(pattern_iter_output_dir, 'results_adv.json'), 'w') as fh:
                    json.dump(results_dict, fh)

                for metric, value in eval_result['scores'].items():
                    results_adv[metric][pattern_id].append(value)

                for metric, value in dev32_result['scores'].items():
                    dev32_results_adv[metric][pattern_id].append(value)
    ...
    if do_eval:
        ...
        _write_results(os.path.join(output_dir, 'result_test.txt'), results, dev32_results)
    if do_eval_adv:
        logger.info("=== OVERALL ADV RESULTS ===")
        _write_results(os.path.join(output_dir, 'result_test_adv.txt'), results_adv, dev32_results_adv)
    


def train_single_model_adv(train_data: List[InputExample],
                       eval_data: List[InputExample],
                       dev32_data: List[InputExample],
                       pattern_iter_output_dir: str,
                       model: RobustWrapper,
    ...
    if not train_data:
        ...
        global_step, tr_loss = model.robust_train(
            pattern_iter_output_dir=pattern_iter_output_dir,
            eval_config=eval_config,
            train_data=train_data,
            dev32_data=dev32_data,
            ...
            max_grad_norm=config.max_grad_norm,
            alpha=config.alpha,
            adv_lr=adv_config.adv_lr,
            adv_weight_decay=adv_config.adv_weight_decay,
            adv_ratio=config.adv_ratio
        )
        results_dict['global_step'] = global_step
        results_dict['average_loss'] = tr_loss

    model = RobustWrapper.from_pretrained(pattern_iter_output_dir)
    results_dict['train_set_after_training'] = evaluate(model, train_data, eval_config)['scores']['acc']
    results_dict['train_set_after_training_adv'] = evaluate_adv(model, train_data, eval_config)['scores']['acc']
    return results_dict
...
def evaluate_adv(model: RobustWrapper,
             eval_data: List[InputExample],
             config: EvalConfig) -> Dict:

    metrics = config.metrics if config.metrics else ['acc']
    results = model.eval_adv(eval_data=eval_data,
                         per_gpu_eval_batch_size=config.per_gpu_eval_batch_size,
                         n_gpu=config.n_gpu,
                         calibrate=config.calibrate)
    predictions = np.argmax(results['logits'], axis=1)
