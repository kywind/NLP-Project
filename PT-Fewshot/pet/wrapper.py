...
from pet.utils import InputExample as PETInputExample
...
class TransformerModelWrapper:
    ...
    def calibrate(self, example, calibrate):
        print('Calibrating...')
        content_free_inputs = ['N/A', 'N/A N/A N/A', '[MASK] [MASK] [MASK]', '[MASK]']
        probs = []
        for cfi_a in content_free_inputs:
            for cfi_b in content_free_inputs:
                # construct input example and feature
                input_example = PETInputExample(
                    guid=example.guid,
                    text_a=cfi_a,
                    text_b=cfi_b,
                    label=example.label,
                ...
                eval_dataset = self._generate_dataset([input_example])
                eval_batch_size = 1
                eval_sampler = SequentialSampler(eval_dataset)
                eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=eval_batch_size)

                # feed into model, get logits
                for batch in eval_dataloader:  # only one batch
                    self.model.eval()
                    batch = {k: t.cuda() for k, t in batch.items()}
                    with torch.no_grad():
        ...
        # get probabilities, num_classes
        probs = torch.cat(probs, dim=0).mean(0)
        num_classes = probs.shape[0]

        if not calibrate:
            W = torch.eye(num_classes).cuda()
            b = torch.zeros((num_classes, 1)).cuda()
        else:
            # diagonal W
            W = torch.linalg.inv(torch.eye(num_classes).cuda() * probs)
            b = torch.zeros((num_classes, 1)).cuda()

            # identity W
            # W = torch.eye(num_classes).cuda()
            # b = -1 * probs.unsqueeze(-1)
    ...
    def eval_dev(self, dev_data, eval_config, n_gpu):
        ...
        results = self.eval(dev_data,
                            ...
                            n_gpu=n_gpu,
                            calibrate=eval_config.calibrate)
    ...
    def eval(self,
        ...
        W, b = self.calibrate(eval_data[0], calibrate)
        print(W)
    ...
    def _generate_dataset(self, data: List[InputExample], labelled: bool = True):
        ...
        feature_dict = {
            ...
            'a_flag': torch.tensor([f.a_flag for f in features], dtype=torch.long),
            'b_flag': torch.tensor([f.b_flag for f in features], dtype=torch.long)
        }
    ...
    def generate_default_inputs(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.config.model_type == "albert":
            self.raw_embeds = model.model.albert.embeddings.word_embeddings(input_ids)
        elif self.config.model_type == "bert":
            self.raw_embeds = model.model.bert.embeddings.word_embeddings(input_ids)
        elif self.config.model_type == "roberta":
            self.raw_embeds = model.model.roberta.embeddings.word_embeddings(input_ids)
        ...
        for bidx in range(bz):
            for i in range(blocked_indices.shape[1]):
                self.raw_embeds[bidx, blocked_indices[bidx, i], :] = replace_embeds[i, :]

        inputs = {'inputs_embeds': self.raw_embeds, 'attention_mask': batch['attention_mask']}
    ...
    def generate_dataset(self, data: List[InputExample], labelled: bool = True):
        return self._generate_dataset(data, labelled)
...
class Adversarial():
    def __init__(self, adv_config):
        ...
        self.model = nn.Linear(1,1) # a default model
        self.optim = optim.SGD(self.model.parameters(), lr=1e-3) # a default optimizer
    ...
    def get_adversarial(self, input_example: List[InputExample])->Dict:
        raise NotImplementedError
    
    def train_adversarial_step(self, input_example: List[InputExample], wrapper: TransformerModelWrapper)->Dict:
        # adv_example = self.get_adversarial(input_example)['adv_example']
        ...
        #         prediction_scores = logits.float().cuda()
        #     adv_loss_total = 0
        #     # adv_loss_total = self.get_adversarial_loss(prediction_scores, labels)['adv_loss_total']
    ...
    def get_eval_function():
        return EVALUATION_STEP_FUNCTIONS[MLM_WRAPPER]





class RobustWrapper(TransformerModelWrapper):
    def __init__(self, config: WrapperConfig, adversarial: Adversarial):
        super().__init__(config)
        self.adversarial = adversarial
        self.adversarial.model.cuda()
    
    def _generate_adv_dataset(self, data: List[InputExample], labelled: bool = True, p=1.0):
        t = np.random.uniform(0.0, 1.0, (len(data), ))
        mask = t < p*np.ones((len(data), ))

        adv_data, non_adv_data = [], []
        for i in range(len(data)):
            (adv_data if mask[i] else non_adv_data).append(data[i])
        
        adv_data = self.adversarial.get_adversarial(adv_data, self.adversarial.attack1, self.adversarial.attack2)['adv_example']
        # print(adv_data[0].text_a)
        # print(adv_data[0].text_b)
        features = self._convert_examples_to_features(adv_data + non_adv_data, labelled=labelled)
        feature_dict = {
            'input_ids': torch.tensor([f.input_ids for f in features], dtype=torch.long),
            'attention_mask': torch.tensor([f.attention_mask for f in features], dtype=torch.long),
            'token_type_ids': torch.tensor([f.token_type_ids for f in features], dtype=torch.long),
    ...
    def eval_adv(self,
        ...
        eval_dataset = self._generate_adv_dataset(eval_data)
        eval_batch_size = per_gpu_eval_batch_size * max(1, n_gpu)
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=eval_batch_size)
    ...
    def train_adv(self,
              train_data:List[InputExample],
              train_adv_batch_size: int=8,
              train_adv_epoch: int=1,
              adv_lr: float=1e-4,
              adv_weight_decay: float=0.0, **_) -> Dict:

        train_example_dataset = ListDataset(train_data)
        train_example_sampler = RandomSampler(train_example_dataset)
        train_example_loader = DataLoader(train_example_dataset, sampler=train_example_sampler, batch_size=train_adv_batch_size, collate_fn=lambda x:x)

        adv_optimizer = AdamW(self.adversarial.model.parameters(), lr=adv_lr, weight_decay=adv_weight_decay)
        self.adversarial.set_optimizer(adv_optimizer)

        for n_epoch in range(train_adv_epoch):
            adv_loss_list = []
            cnt = 0
            for batch in tqdm(train_example_loader, desc="Training Adversarial, Epoch {:02d}/{:02d}".format(n_epoch, train_adv_epoch)):
                cnt += 1
                self.model.eval()
                adv_loss = self.adversarial.train_adversarial_step(batch, self)['adv_loss_total']
                adv_loss_list.append(float(adv_loss))
                if cnt >= 10:
                    break
            logger.info("epoch {}, adv loss {}".format(n_epoch, adv_loss_list))
    ...
    def robust_train(self,
              ...
              max_steps=-1,
              adv_lr: float=1e-4,
              adv_weight_decay: float=0.0,
              adv_ratio: float=0.0, **_):
        """
        Train the underlying language model.
        ...
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=train_batch_size, collate_fn=lambda x:x)
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                # train adv
                if not self.adversarial.no_training:
                    self.model.eval()
                    self.adversarial.train_adversarial_step(batch, self)
                
                # print(batch)

        

                # train model on adv data
                self.model.train()

                # just reuse the preprocess
                batch_dataset = ListDataset(self._generate_adv_dataset(batch, p=adv_ratio))
                batch_dataset_sampler = SequentialSampler(batch_dataset)
                batch_loader = DataLoader(batch_dataset, sampler=batch_dataset_sampler, batch_size=len(batch))
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    ## TODO
                    if global_step % self.config.eval_every_step == 0:
                        dev32_scores = self.eval_dev_adv(dev32_data, eval_config, n_gpu)
    ...
    def save(self, path: str) -> None:
        ...
        state['adv'] = self.adversarial

        save_path_file = os.path.join(path, "embeddings.pth")
        torch.save(state, save_path_file)

    @classmethod
    def from_pretrained(cls, path: str) -> 'RobustWrapper':
        """Load a pretrained wrapper from a given path."""

        wrapper = RobustWrapper.__new__(RobustWrapper)
        wrapper.config = wrapper._load_config(path)
    ...
    def eval_dev_adv(self, dev_data, eval_config, n_gpu):
        self.model.eval()
        results = self.eval_adv(dev_data,
                            per_gpu_eval_batch_size=eval_config.per_gpu_eval_batch_size,
                            n_gpu=n_gpu,
                            calibrate=eval_config.calibrate)
        predictions = np.argmax(results['logits'], axis=1)
