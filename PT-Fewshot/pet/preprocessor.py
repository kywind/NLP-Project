...
class MLMPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, labelled: bool, priming: bool = False,
        ...
        input_ids, token_type_ids, block_flag, a_flag, b_flag = self.pvp.encode(example)
        ...
        a_flag = a_flag + ([0] * padding_length)
        b_flag = b_flag + ([0] * padding_length)
        ...
        assert len(a_flag) == self.wrapper.config.max_seq_length
        assert len(b_flag) == self.wrapper.config.max_seq_length
